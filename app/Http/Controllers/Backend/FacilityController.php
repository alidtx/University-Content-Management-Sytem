...
class FacilityController extends Controller
{
    ...
    {
        // dd('test');
        $data['facility'] = Facility::orderBy('sort_order', 'desc')->get();
    ...
    public function storeFacility(Request $request)
    {
        ...
        $params = $request->all();
        // $new_url = preg_replace('/[^A-Za-z0-9\-]/', '', strtolower(trim($request->title)));
        $params['short_url'] = str_replace(" ", "-", strtolower(trim($request->title)));
        if ($request->image) {
