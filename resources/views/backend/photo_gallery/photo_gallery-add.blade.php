    ...
    <div class="container-fluid">
        ...
        <div class="card">
            <div class="card-header">
                        <a href="{{route('top-menu.photo_gallery', $photo_master_id)}}" class="btn btn-info btn-sm"><i class="fas fa-stream"></i> View Photo Gallery</a>
            ...
            <div class="card-body">
                <form id="" action="{{ !empty($editData)? route('top-menu.photo_gallery.update',$editData->id) : route('top-menu.photo_gallery.store') }}" method="post" enctype="multipart/form-data" autocomplete="off">
                    ...
                    @csrf
                    <input name="photo_master_id" type="hidden" value="{{$photo_master_id}}">
                    ...
                    <div class="form-row">
                        ...
                        {{-- <div class="form-group col-sm-12">
                                @if(!empty($editData->id)? $editData->image : '')
                                    <label>Existing Images</label>
                                    <div class="form-group">
                                        <img src="{{ asset('public/upload/photo_gallery/'.$editData['image']) }}" height="80">
                                        <a href="{{ route('remove.image.from_photo_gallery',$editData->id) }}" class="btn btn-sm btn-danger"><i class="fas fa-trash-alt"></i></a>
                                    </div>
                                @endif
                        </div> --}}
                        <div class="form-group col-sm-12">
                    ...
                    <button class="btn bg-gradient-success btn-sm"><i class="fas fa-save"></i> {{ !empty($editData)? 'Update ' : 'Save' }}</button>
                </form>
