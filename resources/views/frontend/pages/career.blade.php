...
@section('content')
    ...
    <section class="counter-numbers" style="background: url({{ asset('public/frontend/images/banner/faculty.png') }})">
        <div class="container">
            <div class="row">
                <div class="col">
                    <h3 class="text-center my-font" style="color: white;">Career & Job</h3>
                </div>
            </div>
        </div>
    </section>
                ...
                <div class="col-12 col-sm-6" style="background-color: #00B2FF; margin: 40px 0;">
                    <div class="row">
                        <div class="col-10 offset-0 offset-sm-2">
                            <h3 class="title-text my-font" style="margin-left:-102px">Career & Jobs</h3>
                        </div>
                    </div>
                </div>
            </div>
                                        ...
                                        <td style="text-align: center;">
                                            @php
                                                if ($career->attachment != null) {
                                                    $ext = explode('.', $career->attachment);
                                                }
                                            @endphp
                                           
                                            @if ($career->attachment != null && $ext[1] == 'pdf')
                                                <a target="_blank"
                                                    href="{{ asset('public/upload/career/' . $career->attachment) }}"><img
                                                        src="{{ asset('public/frontend/images/pdf_icon.png') }}"
                                                        height="40"></a>
                                            @endif
                                            @if ($career->attachment != null && ($ext[1] == 'doc' || $ext[1] == 'docm' || $ext[1] == 'docx'))
                                                <a target="_blank"
                                                    href="{{ asset('public/upload/career/' . $career->attachment) }}"><img
                        ...
                        <div class="card-body d-flex flex-column">
                            <ul class="list-unstyled mt-3 mt-sm-0">
                                <li class=" mb-3">
                                        <div class="row">
                                            <div class="col-1">
                                                <img src="{{ asset('public/upload/career/'.$career->image) }}" onerror="this.onerror=null;this.src='{{ asset('public/frontend/cuimages/dummy.png') }}';" alt="" class="img-fluid">
                                            </div>
                                            <div class="col-9">
                                                <h4>{{$career->title}}</h4>
                                                <b><small> {{mydate($career->start_date)}}</small></b> - <b><small> {{mydate($career->end_date)}}</small></b>
                                                <p>{!! $career->description !!}</p>
                                            </div>
                                            <div class="col-1">
                                                @if ($career->attachment != null)
                                                <a href="{{ asset('public/upload/career/'.$career->attachment) }}"><img src="{{ asset('public/frontend/images/pdf_icon.png') }}" height="40"></a>
                                                @endif
                                            </div>
                                        </div>
                                </li>
