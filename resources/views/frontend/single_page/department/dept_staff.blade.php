...
@php
$pageTitle='Officers & Staff - Department of '. @$department->name
@endphp
...
{{-- <section class="counter-numbers"
  ...
  <div class="container">
    <div class="row">
      <div class="col">
        <h3 class="text-center my-font" style="color: white;">Office Staff of {{ @$department->name }}</h3>
      </div>
    </div>
  </div>
</section>
          ...
          <div class="col-12 col-sm-6" style="background-color: #00B2FF; margin: 40px 0;">
            <div class="row">
              <div class="col-10 offset-0 offset-sm-2">
                <h3 class="title-text my-font">Office Staff</h3>
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="card-body d-flex flex-column">
            <div class="col-md-12 position-relative">
              <div class="round-image">
                <img
                  src="{{ @$member->member->image ? asset('public/upload/members/'.@$member->member->image) : asset('public/upload/members/member.jpeg') }}"
                  onerror="this.onerror=null;this.src='{{ asset('public/frontend/cuimages/user-dummy.jpeg') }}';"
                  alt="Image" />
              </div>
            </div>
            <div class="heading-content ">
              {{-- <h3 class="my-font"> Dean Of {{@$member->name}}</span></h3> --}}
              <h3 style="padding-top: 15px; margin-bottom: -5px; min-height: 50px;" class="pro_name">
                {{@$member->member->name}}</h3>
              <h5 style="padding-top: 0px; margin-bottom: 5px;" class="my-font desgination">
                {{@$member->member->member_designation}}</h5><br>
              <small style="font-size: 90%">Cell Phone: {{@$member->member->phone}}</small><br>
